 ...
 * @param {*} callback 
 */
function getHistoryAll (userid, callback) {
  models.User.findAll().then(function(userList) {
    ...
    const historyMap = parseHistoryToObject(historyList)
    return callback(null, historyMap)
  })
  
}

/**
 * 履歴取得。全員分取れるように`getHistoryAll`を用意したのでそちらにつなげ変える必要があるかも
 * @param {*} userid 
 * @param {*} callback 
 * @deprecated getHistoryAll
 */
...
function historyGet (req, res) {
  if (req.isAuthenticated()) {
    getHistoryAll(req.user.id, function (err, history) {
